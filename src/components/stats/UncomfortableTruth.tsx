    ...
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
      <div className="flex items-start justify-between mb-1">
        ...
        {monthsAgo !== null && phase === 'idle' && (
          <span className="text-xs text-slate-500">Last viewed: {monthsAgo}mo ago</span>
        )}
      </div>
      <p className="text-xs text-slate-400 mb-5">Honest, unfiltered facts about your spending over all available history.</p>
        ...
        <div className="bg-slate-900 border border-amber-700/40 rounded-xl p-5">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm text-slate-300 transition"
            >
              Not now
            </button>
          </div>
      ...
      {phase === 'revealed' && (
        <>
          {noKey && (
            <p className="text-xs text-amber-400 mb-3">No AI key found — showing rule-based insights. Add a key in Settings for richer narratives.</p>
          )}
          {error && <p className="text-red-400 text-xs mb-3">{error}</p>}

          <div className="space-y-3 mb-6">
            {statements.map((s, i) => (
              <div key={i} className="bg-slate-900 border-l-2 border-amber-500 rounded-r-lg px-4 py-3">
          ...
          <div className="border-t border-slate-700 pt-4">
            <p className="text-xs text-slate-400 mb-3">Did this change how you think about your spending?</p>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
                    feedback === opt
                      ...
                      : 'border-slate-600 text-slate-400 hover:border-amber-500 hover:text-white'
                  }`}
                >
      ...
      {phase === 'feedback-done' && (
        <>
          ...
          <p className="text-xs text-slate-500">Feedback saved. Thanks for being honest with yourself.</p>
          <button
            onClick={() => setPhase('idle')}
